...
populateShinyApp <- function(shinyDirectory,
  ...
  if(missing(shinyDirectory)){
    shinyDirectory <- system.file("shiny", "PLPViewer", package = "ERSPCdiagnosisdeathAndTreatedRightways")
  }
...
viewShiny <- function(package = NULL){
  if(is.null(package)){
    appDir <- system.file("shiny", "PLPViewer", package = "ERSPCdiagnosisdeathAndTreatedRightways")
  }
  if(!is.null(package)){
